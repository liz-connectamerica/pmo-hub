...
Opening a program lands you on a read-only **View** — a stats strip (linked and Active project counts, a RAG mini-bar, average progress across Active projects, late and on-hold counts), the description/objective/roles, a linked-projects list grouped by stage, and an **Upcoming & late milestones** list pulling from every linked project (not just Active) — click one to jump straight to that project's Milestones tab. If you can edit the program, an **Edit** button swaps in the editable form, where you can also add/remove linked projects and drag to set a program-scoped priority order. Editing a program (or the projects linked to it) is limited to a PMO Admin or that program's own Program Manager, Sponsor, or Business Owner — see the Admin Guide's Programs section for exactly what that covers.
