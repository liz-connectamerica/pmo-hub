...
One thing worth knowing: only an admin can reassign a project's **Owner**, and only an admin can delete a project — both of those are unavailable even to the current owner (Delete lives next to the ownership strip's Edit button, at the top of Information).
...
- **My Capacity** — a card at the top lets you self-report your **BAU (non-project) %**, roughly how much of your time normally goes to non-project work. Below that, **Previous / Current / Next month** tabs show an **Estimated total load** for whichever month you pick — your BAU % plus the allocation % for every project you're on the team for that's active that month, plus a prorated share of your open work requests — with a breakdown line spelling out how the total was built. Each project below it shows its allocation tier and computed %, with an edit icon to **override that % for yourself** if it doesn't match reality (this always wins over the computed default, and never changes the tier itself — that's still set from the project's own Team tab). A project not shown for the selected month is on hold, completed, unscheduled, or just not active that month; a note at the bottom says how many are hidden that way.
