...
**Needs your attention** leads the page: any project you own that's past its target end date, an **Active** project you own that hasn't had its data confirmed within the admin-set threshold, overdue milestones on projects you own, tasks and to-dos assigned to you due within 3 days (or overdue), high-severity open RAID items on projects you own, any open risk, issue, or dependency assigned to you personally on any project (not just ones you own — Assumptions don't have a real "open" state, so they're not included), an open blocker assigned to you personally on any project, and your work request status — a request sent back to you needing more detail (or, for admins, counts of project/work requests awaiting review). Click any item to jump straight to it. Below that, **Your projects** is a compact strip of the active projects you own or sponsor, and admins get a **Portfolio pulse** strip (active count, RAG split, projects missing an owner or sponsor) linking to the full [Portfolio Health](../admin-guide/#portfolio-health) page.
...
Every project that isn't yet completed (active, planned, backlog, or on hold), grouped into collapsible sections by Value Area — each with a quick On Track/At Risk/Blocked pulse in its header, and a Collapse all / Expand all shortcut above the list. Each section is a compact table: health, project name (with its tags as chips underneath, if it has any), category, stage (with the hold reason inline, if on hold), progress, owner, and target end date. Search by name, and filter by Category, Stage, Owner, Tag, or Commitment. Click a row to open that project. Like Roadmap, this is a browsing view — there's no inline editing here.

## Programs

A program groups a set of related projects together, with its own ID (P1, P2…), name, description, business objective, and three named roles: Program Sponsor, Program Manager, and Business Owner. Anyone can browse the Programs list (searchable, sortable columns, avatar initials for each role, and a RAG-dot pill showing the worst status among that program's Active projects).

Opening a program lands you on a read-only **View** — a stats strip (linked and Active project counts, a RAG mini-bar, average progress across Active projects, late and on-hold counts), the description/objective/roles, a linked-projects list grouped by stage, an **Upcoming & late milestones** list pulling from every linked project (not just Active) — click one to jump straight to that project's Milestones tab — and a **Timeline** at the bottom: the same month-window Gantt view as Roadmap, scoped to this program's own linked projects, with the same **Next 12 months / Last 12 months / Specific year** picker. If you can edit the program, an **Edit** button swaps in the editable form, where you can also add/remove linked projects and drag to set a program-scoped priority order. Editing a program (or the projects linked to it) is limited to a PMO Admin or that program's own Program Manager, Sponsor, or Business Owner — see the Admin Guide's Programs section for exactly what that covers.
...
Opening a project gives you tabs across the top: **Information, People, Milestones, Plan, To-Do, RAID log, Documentation,** and **Summarize**.

- **Information** — everything about the project itself, as one scrollable page. A left-hand nav jumps you to any of six sections without leaving the page: **Identity & Classification** (name, description, commitment — Must/Should/Want/Won't, admin-only to change, defaulting to "Needs commitment" — value area, T-shirt size, business unit, delivery methodology, project ID, categories, tags), **Schedule, Stage & Lifecycle** (stage, status, phase, start/target-end dates, the milestone timeline, and the Put on hold / Resume / Mark complete actions), **Progress & Health** (progress %, health, plus an open-blocker summary with a View blockers button linking to Documentation's Blockers tab), **Financials** (estimated value, cost estimate, confidence ratings — only appears if you have financial-view permission; today that's admins only, so most users won't see dollar figures anywhere in the app, even on a project they own), **Relationships** (Depends on, Program — with its own Add/Change picker, since a project can only belong to one at a time — and the originating request if there is one), and **System & Audit** (created date, last edited, and a **Data confirmed** line). A **Change Log** section — a read-only audit trail of who changed what, when, with financial changes deliberately excluded to keep dollar figures from leaking to anyone without financial-view permission — sits at the very bottom. Everyone can see all of it; if you can edit the project, a single **Edit** button sits above that left-hand nav (so it's visible no matter which section you've scrolled to) and puts every section you have permission to edit into a form at once — Financials only joins in if you separately have financial-view permission. It becomes **Save**/**Cancel** while you're editing, saving everything together — only an admin, the project **owner**, or its **sponsor** can change its stage (put on hold, resume, mark complete), from buttons on Schedule, Stage & Lifecycle that are there when you're not mid-edit. Sponsor, Owner, and Requirements Owner live on the **People** tab instead.

If you can edit the project, System & Audit also has a **Confirm still accurate** button — it logs that you checked the project's data and it's still correct, with an optional note, without changing any fields. A badge next to the project's status pills shows how fresh that is (quiet green if recent, amber "needs review" once it's been over 60 days, neutral if it's never been confirmed). Making a real edit anywhere on the Information tab counts as a confirmation too — fixing something is itself a way of validating it — and every confirmation, explicit or via an edit, shows up in the Change Log alongside real field changes.
- **People** — **Sponsor, Owner,** and **Requirements Owner** sit at the top, followed by who's on the team. Reassigning Sponsor or Owner is admin-only, but a project's own **owner** can set its **Requirements Owner** themselves — an Edit button there turns the section into a form in place, the same Save/Cancel pattern as Information (an owner's Edit form only offers the Requirements Owner field; Sponsor and Owner show read-only next to it). Whoever is set as Requirements Owner can then add and manage that project's Requirements and Scope items on the Documentation tab, alongside the owner, sponsor, and admin. Only the owner, sponsor, or an admin can add or remove team members. Each row also has an **allocation tier** dropdown (Owner/Lead, Core, Light touch, or Not set) — set by the owner, sponsor, or an admin — that says roughly how much of that person's time this project is expected to take; it feeds directly into their load on the [Capacity](../admin-guide/#capacity) page. An owner who wants their own time counted needs to be on the team themselves, same as anyone else — there's no separate "owner allocation" field. Click the **(i)** next to "Team members" for a quick reminder of what each tier assumes.
- **Milestones** — anyone can view the list and a milestone's history. Adding, editing, completing, or deleting a milestone is owner/sponsor/admin-only.
- **Plan** (formerly "Tasks") — anyone can browse, search, filter, and expand a task to see its description, checklist, comments, and history. A task has a title, an optional longer description (its own expandable row, not shown inline), an assignee, a status (To Do, In Progress, On Hold, or Done), start/end dates or a working-day duration, an optional dependency on another task, and tags. A new task starts **Unassigned** by default — assigning someone opens a search-and-pick list of people and teams (with an explicit "Unassigned" option), same pattern as picking a program. Tasks can be nested into a hierarchy — a task can be dragged to reorder it among its siblings, or **Promote**/**Demote** (in the **⋮** menu) indents it under the one above it or moves it back out, the same way Microsoft Planner/Project handle sub-tasks; a summary task (one with subtasks) takes its Start/End automatically from the earliest/latest dates among its children, and those fields lock once it has children. A task can instead depend on another task in the same project — set a working-day **Duration** and pick a predecessor, and its Start/End auto-calculate as the next working day after that predecessor ends. Each task shows a sequential **ID** based on its position in that outline, and a parent task can be collapsed to hide its subtasks. (Dragging, Promote, and Demote are only available when no search or filter is active, since they act on the task's true position — searching or filtering shows a flat, unindented list instead, and dragging a task onto one at a different level is rejected — reorder within the same level, or use Promote/Demote to change levels.) The **⋮** menu also has **Add task before** / **Add task after** (inserts a new sibling task right next to this one) alongside Edit and Delete. Deleting a task with subtasks deletes the whole subtree. The **collapsible Timeline** above the task list mirrors that same list exactly — same order, same indentation, same expand/collapse state, so a drag, a promote/demote, or collapsing a parent shows up in the timeline too. A task with dates falling in the current window gets a bar; one with dates outside the window shows "Outside this range," and one with no dates at all shows "No dates set" — either way its row still appears so the hierarchy stays intact. Default is a 1-month window starting today, with 1/2/3-month and custom-range options. A **Set baseline** button (owner/sponsor/admin-only) snapshots every task's current dates — past baselines are kept, never overwritten, and the timeline's **Compare to baseline** dropdown overlays a saved baseline as a dashed ghost bar against the live bar to show plan-vs-actual drift. A **List / Grid** toggle at the top switches the whole view: **Grid** is a dense, spreadsheet-style table (owner/sponsor/admin-only to edit) where every field — title, assignee, status, start/end, duration, depends on — is an inline input or dropdown that saves the moment you change it, plus a row at the bottom for typing in several new tasks in a row without opening the Add Task dialog each time; it's meant for fast bulk entry and editing, not restructuring the outline, so promote/demote/drag and the Timeline stay List-only. **Anyone can post a comment on a task**, regardless of whether they own the project. A circle to the left of a task's title is the completion toggle — if the task is assigned to you, clicking it (with an optional closing comment) marks it done even if you're not the project owner, and clicking a completed task's checkmark reopens it. You can also check off items on that task's **checklist** even though you can't add or remove checklist items yourself. Creating, editing, or deleting a task itself — including adding/removing checklist items, tags, and promoting/demoting — is owner/sponsor/admin-only.
- **To-Do** — a lightweight list for action items, follow-ups, access requests, reminders, and other work that doesn't belong on the project plan. A to-do has just a title, an optional description, an individual assignee, a due date, and a status — **Not Started** (the default for a new to-do), **In Progress**, or **Done** — no hierarchy, scheduling, or checklist. Anyone can browse, search, sort, filter, comment, and expand a to-do's history; creating, editing, or deleting one is owner/sponsor/admin-only, but the assignee can always mark their own to-do done (or reopen it, which sends it back to In Progress, since Done-but-not-really means it was worked on already) themselves — moving from Not Started to In Progress otherwise takes editing the to-do. Every to-do you're assigned also shows up in your **My Tasks** sidebar page, under its own To-Do tab — both Not Started and In Progress count as "open" there, alongside Completed.
- **RAID log** — a left-hand sub-nav (styled like Information's) splits **Risks, Assumptions, Issues,** and **Dependencies** into their own sections, one at a time, each with a badge showing how many of its items are still open (not Closed for risks/issues, not Resolved for dependencies; assumptions have no such status, so their count is everything on record). Everyone can view and expand an item's history, but adding or editing an item is owner/sponsor/admin-only. Unlike Tasks, there's no exception here for someone the item might concern — if you're not the owner or sponsor, you can't log a risk yourself; you'd need to ask one of them or an admin to add it. An open risk, issue, or dependency you're set as the **Owner** of also shows up in your own [Needs your attention](#home). Issues have an optional free-form **Impact** field, for describing what happens if the issue goes unresolved. Risks have the same kind of field — labeled **Impact description** there, to keep it distinct from a Risk's separate High/Medium/Low **Impact** and **Probability** ratings, which combine into that risk's effective severity on the same scale Issues use.
- **Documentation** — a left-hand sub-nav (styled like Information's) splits this tab into six independent sections: **Requirements** — add an item with a title, an optional description, and a status (**Planned**, the default, **In Progress**, **Completed**, **Deferred**, or **Cancelled**); marking one Completed stamps today's date automatically, and moving it off Completed clears that date. A badge shows how many items are still Planned or In Progress. **Scope** works the same way, but splits into two tabs of its own — **In Scope** (identical to Requirements: title, description, the same status workflow) and **Out of Scope**, which drops the status workflow entirely — an item there is simply excluded, with an optional note on why. A single **Add scope item** button sits above both tabs and adds to whichever one is currently open; a toggle inside the form also lets you reclassify an item between the two after the fact. Anyone can view, comment, and expand an item's change log on either tab; adding, editing, or deleting an item is owner/sponsor/admin-only — plus a project's **Requirements Owner** (see People, below), who can manage both Requirements and Scope even without being the project owner. **Decisions** is a simple log — a decision, an optional rationale, who decided it (an optional picker over the resource roster, not free text), and the date — with no status workflow, since a decision is recorded once rather than tracked through a lifecycle. **Meeting Minutes** captures a title, date/time, attendees (multi-select from the resource roster), and a free-form recap, with its own search box that matches on title, recap, or attendee name. **Blockers** replaces what used to be a single free-text field on Progress & Health — each blocker is its own item with a title, a reason (a fixed picklist — Resource/Capacity, Dependency, Decision Required, and so on), blocked-since and target-resolution dates, an owner (from the resource roster), a Yes/No escalation-required flag, its own comment thread, and a change log, split into **Open** and **Resolved** tabs with a badge showing the open count. A blocker assigned to you also shows up in your own [Needs your attention](#home). **Attachments** is the folder/link structure this tab used to be entirely, with no open-count badge since it has no status — everyone can browse folders and open documents. Everyone can view all six sections; adding, editing, or deleting in Decisions, Meeting Minutes, Blockers, or Attachments is owner/sponsor/admin-only.
- **Summarize** — generates a live, stakeholder-ready status report from the project's current data: stage/status/commitment, owner/sponsor/target end, progress, milestones completed since a chosen date vs. upcoming (with late ones flagged), a Requirements/Scope/Plan completion snapshot, and open RAID risks/issues by severity. Financial data is never included. Owner, sponsor, and admin can also fill in three free-form narrative fields — **Executive Summary, Asks & Decisions Needed,** and **What's Next** — which autosave as you click away and always hold just the current draft (no history is kept). Everyone who can view the project can see the report preview and use **Copy for email** (copies the formatted report so it pastes correctly into Outlook via right-click → Paste Special → Keep Source Formatting) and **Download PDF** (uses your browser's print dialog).

One thing worth knowing: only an admin can reassign a project's **Sponsor** or **Owner**, and only an admin can delete a project — those are unavailable even to the current owner (Delete lives next to the Edit button in Information's Identity & Classification section). The owner can, however, set the project's **Requirements Owner** themselves.
